...
func runReverse(cmd *Command, args []string) {
	...
	if err == nil && !info.IsDir() {
		...
		if j, ok := configs["omitempty"]; ok {
			omitempty, err = strconv.ParseBool(j)
		}
	...
	filepath.Walk(dir, func(f string, info os.FileInfo, err error) error {
		...
		if !isMultiFile {
			...
			t := &Tmpl{Tables: tbls, Imports: imports, Model: model, Path: getPath()}
			...
			for _, table := range tables {
				...
				t := &Tmpl{Tables: tbs, Imports: imports, Model: model, Path: getPath()}
				err = tmpl.Execute(newbytes, t)
				if err != nil {
					log.Errorf("%v", err)
					return err
